...
def handleOpen(device, sendEvent=true)
{
	def bondId = getBondIdFromDevice(device)
	logDebug "Handling Open event for ${bondId}"
	
    if (executeAction(bondId, "Open")) {
        if (sendEvent) device.sendEvent(name: "windowShade", value: "open")
    }
}

def handleClose(device, sendEvent=true)
{
	def bondId = getBondIdFromDevice(device)
	logDebug "Handling Close event for ${bondId}"
	
    if (executeAction(bondId, "Close")) {
        if (sendEvent) device.sendEvent(name: "windowShade", value: "closed")
    }
}
